...
data_balanced_both <- ovun.sample(Attrition ~ ., data = data1, method = "both", p=0.5,N=18362, seed = 1)$data
table(data_balanced_both$Attrition)
x=model.matrix(Attrition~.,data_balanced_both)[,-1]
dim(x)
y=data_balanced_both$Attrition
...
#plots
#vip
vip(ridge.model, num_features = 50, geom = "point")
